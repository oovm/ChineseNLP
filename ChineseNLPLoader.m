...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated ChineseNLP loader file *)


Unprotect[ChineseNLP`PackageScope`Private`$TopLevelLoad];
ChineseNLP`PackageScope`Private`$TopLevelLoad=MemberQ[$ContextPath, "Global`"];


BeginPackage["ChineseNLP`"];


ClearAll[ChineseNLP];
ChineseNLP::usage="ChineseNLP is a manager head for the ChineseNLP package";
...
ChineseNLP["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
ChineseNLP["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
ChineseNLP["Contexts"]:=$PackageContexts;
If[!ListQ@$PackageContexts,
  $PackageContexts=
    {
      "ChineseNLP`",
      "ChineseNLP`PackageScope`Private`",
      "ChineseNLP`PackageScope`Package`"
      }
  ];
$PackageDeclared=
...
ChineseNLP["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
ChineseNLP["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
ChineseNLP["AllowRescoping"]:=$AllowPackageRescoping;
$AllowPackageRescoping=
  Replace[
    Lookup[$PackageLoadSpecs, "AllowRescoping"],
    Except[True|False]->$TopLevelLoad
    ];
ChineseNLP["AllowRecoloring"]:=$AllowPackageRecoloring;
$AllowPackageRecoloring=
  Replace[
    Lookup[$PackageLoadSpecs, "AllowRecoloring"],
    Except[True|False]->$TopLevelLoad
    ];
ChineseNLP["AllowAutocompletions"]:=$AllowPackageAutocompletions;
...
ChineseNLP["FileContexts"]:=$PackageFileContexts;
...
ChineseNLP["DeclaredPackages"]:=$DeclaredPackages;
...
ChineseNLP["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,ChineseNLP],"`"]
    ];
...
ChineseNLP::nodep="Couldn't load dependency `` of type ``";
ChineseNLP::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[ChineseNLP::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[ChineseNLP::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[ChineseNLP::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
$PackageErrorMessage=
  "ChineseNLP encountered exception ``";
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[ChineseNLP, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[ChineseNLP, tag], body];
    PackageFailureException[
      MessageName[ChineseNLP, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[ChineseNLP, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[ChineseNLP],
          Nest[
            Insert[#, _, {1, 1, 1}]&,
            (HoldPattern[Verbatim[HoldPattern]][
              ChineseNLP[s_String, ___]
              ]:>_),
            n-1
            ]:>s,
          Infinity
...
If[
  (Clear@ChineseNLP`PackageScope`Private`$loadAbort;!#)&@
    ChineseNLP`PackageScope`Private`$loadAbort,
  Unprotect[ChineseNLP`PackageScope`Private`$LoadCompleted];
  ChineseNLP`PackageScope`Private`$LoadCompleted=True;
  ChineseNLP`PackageScope`Private`PackagePostProcessContextPathReassign[]
  ]